...
class CSearchableDropdown<T> extends StatefulWidget {
  ...
  factory CSearchableDropdown.single({
    Key key,
  ...
  /// @param readOnly [bool] whether to let the user choose the value to select or just present the selected value if any.
  /// @param menuBackgroundColor [Color] background color of the menu whether in dialog box or menu mode.
  factory CSearchableDropdown.multiple({
    Key key,
    @required List<DropdownMenuItem<T>> items,
    @required Function onChanged,
  ...
  @override
  _CSearchableDropdownState<T> createState() => new _CSearchableDropdownState();
}
...
class _CSearchableDropdownState<T> extends State<CSearchableDropdown<T>> {
  ...
  @override
  void didUpdateWidget(CSearchableDropdown oldWidget) {
    ...
    return Column(
      ...
      children: <Widget>[
        ...
        Stack(
          children: <Widget>[
            Padding(
              padding: EdgeInsets.all(10.0), //10
              child: result,
            ),
